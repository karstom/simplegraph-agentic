...
| **Capture** — recording a node when risky code changes (`require_documentation.sh`) | Claude Code `Stop` hook — reminds the agent before it finishes |
...
closes that gap for Claude Code. Wired as a `Stop` hook, it runs when the agent
...
- **Nudge once, never trap.** The `stop_hook_active` guard lets the next stop
...
Wire it during `setup.sh` (Claude Code adapter) or later:

```bash
bash scripts/install_doc_hook.sh /path/to/your/project
```

It merges idempotently into `.claude/settings.json`. Verify the hook itself with
`bash scripts/test_require_documentation.sh`. This is the "force capture" layer;
`consistency_check.sh` in CI remains the "force validity" layer.

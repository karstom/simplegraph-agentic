...
| **Duplicate node IDs** from two branches | `sg check` / `consistency_check.sh` fails the build when one ID is defined twice (across `core/` and `shared/`) |
...
| **A wrong node becoming org-wide law** | The MCP server is read-only against `shared/`, so promotion is a deliberate, human-reviewed act; `sg check` / `consistency_check.sh` validates the shared graph and warns on untraceable org-wide nodes |

...
| **`graph_index.md` merge conflicts** across branches | The Quick Index is *derived* — regenerate it with `simplegraph reindex` instead of hand-merging; the output is sorted and order-independent, so either side resolves identically |
| **Duplicate node IDs** from two branches | `simplegraph check` / `consistency_check.sh` fails the build when one ID is defined twice (across `core/` and `shared/`) |
...
| **A wrong node becoming org-wide law** | The MCP server is read-only against `shared/`, so promotion is a deliberate, human-reviewed act; `simplegraph check` / `consistency_check.sh` validates the shared graph and warns on untraceable org-wide nodes |
...
`simplegraph seed` mines the repository deterministically: offline, no API key, full

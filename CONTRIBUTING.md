...
Some tools read a specific file before any task-driven context loading — e.g. Antigravity 2.x reads `AGENTS.md` at session start (IDE v1.20.3+), and most CLIs read `AGENTS.md` / `CLAUDE.md`. Installing the memory section there guarantees a read regardless of task type. Consult your tool's docs for the exact file and precedence.
